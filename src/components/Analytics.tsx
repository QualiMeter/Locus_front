...
import { LLMAnalyticsBlock } from './LLMAnalyticsBlock';
...
export const Analytics: React.FC<Props> = ({
  ...
  if (!site) {
    return <div className={styles.container}>Загрузка данных площадки...</div>;
  }
  ...
  const connectionCost = Number(site?.connection_rub_per_kw ?? 0) * requiredPower;
...
<Section
  icon="🤖"
  title="Аналитика и рекомендации от LLM (OpenRouter)"
>
  <LLMAnalyticsBlock 
    region={region}
    site={site}
