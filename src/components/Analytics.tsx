...
export const Analytics: React.FC<Props> = ({ region, site, form }) => {
  // Приводим все значения из form к числу для предотвращения ошибок TS2362
  const volume = Number(form.volume || 0);
  const workers = Number(form.workers || 0);
  const budget = Number(form.budget || 0);
  const kindergarten = Number(form.kindergarten || 0);
  const housing = Number(form.housing || 0);

  const budgetRub = budget * 1e6;

  const requiredPower = 300 + (volume / 1000) * 400;
  const connectionCost = (Number(site.connection_rub_per_kw) || 4500) * requiredPower;
  const totalLandConn = (Number(site.cost_land) || 0) + connectionCost;
  ...
  const kindergLoad = Number(region.social.kindergarten_load);
      ...
      <div className={styles.container}>
        ...
        <Section icon="⚡" title={`Сетевая инфраструктура — ${site.name}`}>
          <div className={styles.grid}>
            ...
            <InfoItem label="Техприсоединение">
              {fmt(Number(site.connection_rub_per_kw))} руб/кВт → {(connectionCost / 1e6).toFixed(1)} млн ₽
            </InfoItem>
            <div className={styles.fullSpan}>
        ...
        <Section icon="🚚" title="Логистика сырья">
          <div className={styles.grid}>
            <div>
              ...
              <ProgressBar value={Math.max(10, 100 - Number(region.steel_dist) / 5)} color="#5f9ea0" />
            </div>
            <div>
              <InfoItem label="Расстояние до утеплителя">{region.insulation_dist} км</InfoItem>
              <ProgressBar value={Math.max(10, 100 - Number(region.insulation_dist) / 3)} color="#5f9ea0" />
            </div>
          </div>
        </Section>
        ...
        <Section icon="🧮" title="Укрупнённая смета строительства">
          <table className={styles.smetaTable}>
            ...
            <tr><td>Благоустройство (оценка)</td><td>~{Math.round((Number(site.cost_land) / 2000000) * 0.2)} млн ₽</td></tr>
            </tbody>
            <tfoot>
