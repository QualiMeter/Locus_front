...
export const ConceptBoard: React.FC<Props> = ({ region, priority }) => {
  // Гвард: если region или cultural не загружены
  if (!region?.cultural) {
    return <div className={styles.board}>Загрузка концепта...</div>;
  }
  ...
  // Гвард для styles и materials
  const regionMaterials = region.cultural.materials ?? [];

  let materials = [...regionMaterials];
  if (priority === 'techno') {
    materials = ['Сталь', 'Алюминий', 'Стекло', 'Композитные панели'];
  }
  if (priority === 'eco') {
    materials = ['Древесина', 'Камень', 'Эко-панели', 'Озеленение'];
  }
