...
class OpenRouterService {
  ...
  async generateAnalytics(
    ...
    if (this.useMock || !this.apiKey) {
      ...
      return this.generateMockAnalytics(regionData, siteData, formData, topRegions);
    }

    try {
      const prompt = this.buildPrompt(regionData, siteData, formData, topRegions);
      const response = await this.callOpenRouter(prompt);
      return this.parseResponse(response, regionData, siteData, formData, topRegions);
    } catch (error) {
  ...
  private buildPrompt(regionData: any, siteData: any, formData: any, topRegions?: any[]): string {
...
Верни JSON:
{
  ...
  private parseResponse(response: string, regionData: any, siteData: any, formData: any, topRegions?: any[]): LLMAnalytics {
    try {
      ...
      if (jsonMatch) {
        ...
        return {
          ...
          riskRecommendations: parsed.riskRecommendations || this.generateMockRiskRecommendations(regionData, siteData, formData),
          ...
          benchmarking: parsed.benchmarking || this.generateMockBenchmarking(regionData, topRegions),
  ...
  private generateMockAnalytics(regionData: any, siteData: any, formData: any, topRegions?: any[]): LLMAnalytics {
    return {
      investmentVerdict: this.generateMockInvestmentVerdict(regionData),
      riskRecommendations: this.generateMockRiskRecommendations(regionData, siteData, formData),
      ...
      benchmarking: this.generateMockBenchmarking(regionData, topRegions),
  ...
  private generateMockRiskRecommendations(regionData: any, siteData: any, formData: any): string[] {
    const recs = [];
  ...
  private generateMockBenchmarking(regionData: any, topRegions?: any[]): string {
